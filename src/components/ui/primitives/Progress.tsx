    ...
    <div className={cn("flex flex-col gap-1.5", className)}>
      {(label || showValue) && (
        <div className="flex items-center justify-between">
          {label && <span id={`progress-label-${value}`} className="font-mono text-xs uppercase tracking-[0.15em] text-steel-400">{label}</span>}
          {showValue && <span className="font-mono text-xs text-amber-500">{pct}%</span>}
        </div>
      )}
      <BaseProgress.Root value={value} min={min} max={max} aria-labelledby={label ? `progress-label-${value}` : undefined}>
